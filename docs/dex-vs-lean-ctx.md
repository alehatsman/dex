...
All gaps G1–G9, G11, G12 are done. G10 (streaming MCP responses) remains blocked on an SDK gap — `ServerSession` is not injectable into tool handler context in go-sdk v1.4.1. Second-wave gaps N1–N7 (lean-ctx 3.6.x) done as of `bc6d9d8`. Third-wave gaps N8–N13 (lean-ctx 3.6.13–3.7.x) done as of `944cbb8`. Fourth-wave gaps N14–N22 (lean-ctx 3.6.14–3.7.1) open.
...
| N14 | **Structured `map` mode for non-code files** | High | Low-Medium | ☐ open | lean-ctx 3.6.18: `map` mode produces token-efficient outlines without LLM for Markdown (heading tree), JSON (key structure + types), YAML (key hierarchy), TOML (section headers), and lock files (dependency count summaries). dex current: `map` mode queries graph symbols + imports; non-code files with no graph data return "no indexed data". Fix: detect file extension in `view_summarize`; if non-code or no graph symbols, parse in Go and emit structural outline. ~95% token savings vs `full` on large config/doc files. |
| N15 | **Composite `search_compose` tool** | High | Medium | ☐ open | lean-ctx 3.6.25: `ctx_compose(task)` returns keywords, semantically ranked files, exact locations, and the best-matching symbol's body in one call — replacing the typical search→signatures→lines chain. Uses submodular packing for non-redundant symbol bodies under a token budget. dex current: agents call `search_semantic`, then `view_summarize(signatures)`, then `view_summarize(lines:N-M)` — 3 round trips. Fix: add `search_compose(query, project_root, k)` MCP tool that fuses search results, top-file signatures, and best symbol body into a single response. |
| N16 | **Task-relevance inline in `signatures`/`map` mode** | Medium | Medium | ☐ open | lean-ctx 3.6.25: when a session task is declared, `map`/`signatures` mode inlines the body of the best-matching symbol for that task, avoiding a follow-up full read. `map` auto-selection threshold also raised 3000→6000 tokens. dex current: `signatures` lists all symbols with no body content; no task-awareness in read modes. Fix: when session has a declared task and mode is `signatures` or `map`, BM25-match task text against symbols in that file and append the top match's body (capped at N lines). |
| N17 | **Cold-start `overview` partial view** | Medium | Low | ☐ open | lean-ctx 3.6.26: when the graph is still building, `ctx_overview` returns a useful partial — detected project markers (go.mod, package.json, Cargo.toml, etc.), depth-2 gitignore-filtered tree, and persistent knowledge — instead of "INDEXING IN PROGRESS, retry later". dex current: `overview` returns empty/minimal when `chunks` table is unpopulated. Fix: when chunk count is zero, walk root for project markers, emit `search_tree(root, depth=2)` output, prepend knowledge facts, return status `"partial"` with a hint. |
