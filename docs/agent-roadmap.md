...
## 1. ✅ Operational config in `.dex/config.yml` (env-var consolidation)
...
pin them. `.dex/config.yml` now carries `endpoints:`, `models:`, `tools:`, and
an `env:` escape hatch (`cmd/dex/config_file.go`, parsed with `gopkg.in/yaml.v3`
— no hand-rolled parser), resolved at startup from the working dir. Precedence
is strict: **env var > config file > default**, so the dotfiles/systemd env
