...
- WHEN a client queries a project, the per-project routes are flat and share the
  ...
  `.../callees`, `.../impact`, `.../locate`, `.../review`, `.../refactor`, `.../cohort`, `.../routes`,
  `.../smells`, `.../path`, `.../diff`, `.../clusters`,
  `.../session`, plus `GET .../graph/packages` and the global `POST /v1/shell`.
  (The `.../notes` route was removed with the knowledge subsystem in #205.)
...
- [x] Per-project routes share the MCP tool names, flat (ask/map/trace/find/lookup/grep/read/ls/deps/callers/callees/impact/locate/review/refactor/cohort/routes/smells/path/diff/clusters/session + graph/packages + global shell; `notes` removed in #205); structured status in body
