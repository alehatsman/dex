...
func newTestProxy(t *testing.T, upstream http.Handler) (*httptest.Server, *bytes.Buffer) {
	...
	front := httptest.NewServer(newProxyHandler(upURL, logger, &Stats{}, "", ToolDescFull, ModelRouteConfig{}, nil, nil, nil, nil, "", nil, nil, nil))
...
func TestAuthGate(t *testing.T) {
	...
	front := httptest.NewServer(newProxyHandler(upURL, logger, &Stats{}, token, ToolDescFull, ModelRouteConfig{}, nil, nil, nil, nil, "", nil, nil, nil))
	defer front.Close()
...
func TestFeedbackHookFired(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	front := httptest.NewServer(newProxyHandler(upURL, logger, &Stats{}, "", ToolDescFull, ModelRouteConfig{}, nil, nil, nil, nil, "", nil, hook, nil))
	t.Cleanup(front.Close)
