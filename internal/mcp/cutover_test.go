...
// collapsed: exactly two tools headline it, every deleted verb is unreachable at
...
// removedVerbs are the tools deleted across S2–S4 (#196/#197/#198). None may
// register at either the default or the DEX_EXPERT tier — a clean break, no
// aliases, no deprecation window (spec §"no backward compat"):
...
// TestCutoverEverydaySurfaceIsExactlyTwoTools asserts the default surface is
// EXACTLY {query, record} — no more, no less. The pre-existing expert-gating
// tests only check that the two verbs are present and the power lanes absent;
// this pins the set so a stray everyday-tier registration can't slip in.
func TestCutoverEverydaySurfaceIsExactlyTwoTools(t *testing.T) {
	t.Setenv("DEX_EXPERT", "") // default surface, power tier off
	names := listToolNames(t, stubServer(t))

	want := map[string]bool{"query": true, "record": true}
	for n := range names {
		if !want[n] {
			t.Errorf("default surface advertised unexpected tool %q; want exactly {query, record}", n)
		}
	}
	for n := range want {
		if !names[n] {
	...
	if len(names) != len(want) {
		t.Errorf("default surface has %d tools %v; want exactly 2 {query, record}", len(names), toolNameSet(names))
	}
}
...
// TestServerInstructionsBudget asserts the two-verb system prompt is well under
...
func TestServerInstructionsBudget(t *testing.T) {
	const (
		...
		budget          = 600 // two-verb ceiling: ~533 today, comfortably under four verbs'
	)
	...
	// The prompt must still name both verbs — a budget met by dropping guidance
	// would be a false economy.
	for _, v := range []string{"query", "record"} {
