...
type AgentInput struct {
	...
	Action      string `json:"action"                 jsonschema:"announce | post | read | list | diary | recall_diary | list_diaries"`
	...
	Category    string `json:"category,omitempty"     jsonschema:"bus message kind (post/read): finding | plan | error | note; diary entry kind (diary): discovery | decision | blocker | progress | insight — defaults to insight"`
	Query       string `json:"query,omitempty"        jsonschema:"full-text search query over message bodies (used with read; powered by FTS5); when set, results are sorted by relevance rank instead of chronological id order"`
	Body        string `json:"body,omitempty"         jsonschema:"message text — required for post (bus message body) and diary (diary entry content)"`
	SinceID     int64  `json:"since_id,omitempty"     jsonschema:"return messages with id > since_id for incremental polling (used with read; applies before relevance ranking when query is also set)"`
	Limit       int    `json:"limit,omitempty"        jsonschema:"max messages to return (default 50, max 200; used with read and recall_diary)"`
}
...
func (s *Server) agent(ctx context.Context, _ *sdk.CallToolRequest, in AgentInput) (*sdk.CallToolResult, AgentOutput, error) {
	...
	case "diary":
		...
		if in.Body == "" {
			return nil, AgentOutput{Status: "error", Hint: "body is required for diary"}, nil
		}
		cat := in.Category
		if cat == "" {
			cat = "insight"
		}
		entry, err := DiaryAppend(s.IndexDir, in.AgentID, cat, in.Body)
		if err != nil {
	...
	default:
		return nil, AgentOutput{
			Status: "error",
			Hint:   fmt.Sprintf("unknown action %q — want: announce | post | read | list | diary | recall_diary | list_diaries", in.Action),
		}, nil
	}
}
