...
// jsWorkspaceView builds a per-module TS import view: two @acme/ui modules
// importing two @acme/common modules (dedups to one project edge), plus an
...
func jsWorkspaceView() *graphquery.View {
	...
	return &graphquery.View{
		NodesByID: map[string]graphquery.Node{
			...
			"i3":       tsImp("i3", "packages/acme-ui/src/Card", "@acme/ui/Button", "packages/acme-ui/src/Button"), // intra-project
		},
...
func TestEnrichGraphProjectTopology(t *testing.T) {
	...
	for _, proj := range []string{"@acme/ui", "@acme/common"} {
	...
	e := gr.Edges[0]
	if e.From != "@acme/ui" || e.To != "@acme/common" || e.Kind != string(graph.EdgeImports) {
		t.Errorf("edge = %+v, want @acme/ui -imports-> @acme/common", e)
	}
}
...
func TestEnrichGraphProjectTopologyNilMapperFallsBack(t *testing.T) {
	...
	if gr != nil {
		for _, n := range gr.Nodes {
			if strings.HasPrefix(n.ID, "@acme/") {
