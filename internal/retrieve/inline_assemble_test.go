...
// These cases moved down from internal/mcp with the assemble-pool widening when
// #113 relocated the inline orchestration into retrieve. expandAssemblePool now
// takes the role formatter explicitly (nil here — Role isn't asserted).
...
func symPaths(syms []SymbolHit) []string {
...
func TestExpandAssemblePoolSymbolSeed(t *testing.T) {
	...
	got := expandAssemblePool(v, seed, nil, 16, nil)
	if len(got) == 0 || got[0].Path != "a.go" {
		t.Fatalf("seed must lead result, got %v", symPaths(got))
	}
	for _, want := range []string{"b.go", "c.go"} {
		if !containsStr(symPaths(got), want) {
			t.Errorf("neighbor %q not added; got %v", want, symPaths(got))
		}
	}
}
...
func TestExpandAssemblePoolSemanticSeed(t *testing.T) {
	...
	got := expandAssemblePool(v, nil, sem, 16, nil)
	if !containsStr(symPaths(got), "b.go") || !containsStr(symPaths(got), "c.go") {
		t.Errorf("semantic seed did not pull call-neighbors; got %v", symPaths(got))
	}
}
...
func TestExpandAssemblePoolDedup(t *testing.T) {
	...
	got := expandAssemblePool(v, seed, nil, 16, nil)
	n := 0
	for _, p := range symPaths(got) {
		if p == "b.go" {
			n++
		}
	}
	if n != 1 {
		t.Errorf("b.go appears %d times, want 1: %v", n, symPaths(got))
	}
}
...
func TestExpandAssemblePoolCap(t *testing.T) {
	v := callChainView()
	seed := []SymbolHit{{QualifiedName: "A", Path: "a.go", StartLine: 12, EndLine: 12}}
	got := expandAssemblePool(v, seed, nil, 1, nil)
	if len(got) != 2 { // seed + exactly one neighbor
		t.Errorf("maxAdd=1 should yield 1 seed + 1 neighbor, got %d: %v", len(got), symPaths(got))
	}
}

func TestExpandAssemblePoolNilView(t *testing.T) {
	seed := []SymbolHit{{QualifiedName: "A", Path: "a.go", StartLine: 12}}
	if got := expandAssemblePool(nil, seed, nil, 16, nil); len(got) != 1 {
		t.Errorf("nil view must be a no-op, got %v", symPaths(got))
	}
}
