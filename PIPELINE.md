...
`internal/mcp` wraps this for the MCP tool surface. `ask` leads the surface — it composes the lanes above and, when a chat endpoint is configured, synthesizes a grounded prose `answer` (with `path:line` citations) from the evidence bundle (`internal/mcp/answer.go`). The individual lanes are exposed as their own tools (`find`, `lookup`, `callers`, `deps`, `impact`, `clusters`, `read`, `status`, …); the surface is **capability-derived** (#283/#290) — `find` appears only when a query-time embedder is wired, `read` only when a chat model is, and a weak model collapses the surface to `ask`/`grep`/`ls`/`shell`. There are no tiers. `cmd/dex/main.go` provides the CLI mirrors and the MCP stdio server entrypoint.
