...
//
// A fourth subcommand, `rewrite` (PreToolUse Bash — rewrote rg/grep to dex
// commands by literal name), was dropped outright by #849 rather than
// migrated: silently rewriting the agent's own shell commands is exactly the
// "magic" the project's explicit>magic bar argues against, and the CLI verb
// it rewrote toward (`dex search`) no longer exists as a named door anyway.
