...
func cmdKnowledge(ctx context.Context, args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("notes needs a subcommand: add | list | delete | review | pin | unpin | gc")
	}
  ...
  (query/rm are accepted as aliases for list/delete)`)
		return nil
	default:
		return fmt.Errorf("unknown notes subcommand: %s (have: add, list, delete, review, pin, unpin, gc)", sub)
	}
}
