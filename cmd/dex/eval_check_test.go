...
func TestCheckEvalRegression_ManifestGate(t *testing.T) {
	...
	if err := checkEvalRegression(good, writeRefReport(t, ref), false, 0.02); err != nil {
	...
	bm.K = 20
	bad.Manifest = &bm
	err := checkEvalRegression(bad, writeRefReport(t, ref), false, 0.02)
	if err == nil || !strings.Contains(err.Error(), "incompatible") {
		t.Fatalf("expected incompatible-manifest error, got %v", err)
	}

	// Same mismatch but override set → proceeds (metrics ok) → pass.
	if err := checkEvalRegression(bad, writeRefReport(t, ref), true, 0.02); err != nil {
	...
	if err := checkEvalRegression(good, writeRefReport(t, legacy), false, 0.02); err != nil {
...
func TestCheckEvalRegression_PerTypeGate(t *testing.T) {
	...
	err := checkEvalRegression(now, writeRefReport(t, ref), false, 0.02)
