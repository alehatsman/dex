...
func cmdMap(ctx context.Context, args []string) error {
	...
	if *format == "json" {
		...
		return enc.Encode(map[string]any{"zoom": "orient", "map": codemap.RenderOrient(clusters,
			codemap.OrientExtras{Entrypoints: out.Entrypoints, Commands: mcp.ExtractProjectCommands(out.Project), ImportEdges: mcp.CodemapImportEdges(out.ImportEdges), Externals: out.Externals, Scale: mcp.CodemapScale(out.Scale)}, *budget, 0)})
	}
